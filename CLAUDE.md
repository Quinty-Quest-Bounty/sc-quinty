...
Quinty is an on-chain task bounty system for Base network with four interconnected smart contracts:

### Core Contract System

1. **Quinty.sol** - Main bounty contract with 100% ETH escrow, blinded IPFS submissions, winner selection, and automatic slashing (25-50%) on expiry
2. **DisputeResolver.sol** - voting system for expiry disputes and pengadilan (court) disputes with 0.0001 ETH staking
3. **QuintyReputation.sol** - Soulbound ERC-721 NFT reputation system with IPFS metadata and dynamic Bronze/Silver/Gold levels
4. **AirdropBounty.sol** - Transparent promotion task rewards with fixed ETH payouts for verified social proofs
...
- **ETH Token**: Native ETH token used for all escrow, staking, and rewards
...
1. **Bounty Lifecycle**: Create (escrow ETH) → Submit (IPFS + 10% deposit) → Resolve (select winners) → Reveal solutions
...
4. **Dispute Resolution**: Stake 0.0001 ETH → Rank top 3 submissions → Weighted voting → Proportional rewards
...
- **Mock Data**: Tests use realistic ETH amounts and IPFS CID formats
...
- All ETH transfers use native token (msg.value), not ERC-20
...
- Minimum 0.0001 ETH stake required for accessibility
...
- `BASE_SEPOLIA_RPC`: RPC endpoint for Base Sepolia testnet (default: https://sepolia.base.org)
- `BASE_MAINNET_RPC`: RPC endpoint for Base Mainnet (default: https://mainnet.base.org)
- `PRIVATE_KEY`: Deployer private key (optional, fallback to test mnemonic)

## Deployment Info

Contract addresses are saved in `deployments.json` after successful deployment. Use the appropriate network flag when deploying:
